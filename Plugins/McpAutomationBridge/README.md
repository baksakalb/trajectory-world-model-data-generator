...
An Unreal Engine editor plugin that enables Codex and other MCP clients to control Unreal Engine through the Model Context Protocol (MCP).
...
For this project, Codex uses `C:\Users\baris\.codex\config.toml` as the canonical MCP config. Generic MCP clients can use an equivalent server entry:
...
2. Start Codex or another MCP client
